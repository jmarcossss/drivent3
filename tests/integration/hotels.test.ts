...
describe('GET /hotels/:id', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 404 if ticket not found', async () => {
      const newPeople = await createUser();
      const newToken = await generateValidToken(newPeople);
      await createEnrollmentWithAddress(newPeople);
      const hotel = await createHotel();
      await createHotelRoom(hotel.id);
    ...
    it('should respond with status 402 if ticket type is remote', async () => {
      const newPeople = await createUser();
      const newToken = await generateValidToken(newPeople);
      const enrollment = await createEnrollmentWithAddress(newPeople);
    ...
    it('should respond with status 404 if hotel not found', async () => {
      ...
      const response = await server
        .get('/hotels/' + faker.datatype.number())
    ...
    it('should respond with status 400 if hotel id is not a number', async () => {
      ...
      const ticketType = await createNotRemote(true);
      await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      const response = await server.get('/hotels/' + faker.lorem.word()).set('Authorization', `Bearer ${newToken}`);
...
describe('GET /hotels', () => {
  ...
  it('should respond with status 401 if there is no session for given token', async () => {
    const userWithoutSession = await createUser();
    const newToken = jwt.sign({ userId: userWithoutSession.id }, process.env.JWT_SECRET);
    const response = await server.get('/hotels').set('Authorization', `Bearer ${newToken}`);
    expect(response.status).toBe(httpStatus.UNAUTHORIZED);
  });
